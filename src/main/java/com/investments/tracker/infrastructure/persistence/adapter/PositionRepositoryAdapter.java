 ...
 *
 * <p>Position aggregate does not contain price data (ADR-023). Price data lives in the Instrument
 * aggregate and is fetched separately.
 */
@Repository
public class PositionRepositoryAdapter implements PositionRepository {
    ...
    public PositionRepositoryAdapter(
            PositionJdbcRepository jdbcRepository, PositionPersistenceMapper mapper) {
    ...
    public Position save(Position position) {
        Long version =
                jdbcRepository
                        .findById(position.symbol().value())
                        .map(PositionJdbcEntity::version)
                        .orElse(null);
